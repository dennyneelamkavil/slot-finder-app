...
export default async function AvailabilityPage({ searchParams }) {
  ...
  function findSlots() {
    ...
    // Clone of schedules to track usage during this function
    const tempUsage = {};
      ...
      for (let hour = 9; hour < 18; hour++) {
        for (let min of [0, 30]) {
          ...
          const ok = selected.every((id) => {
            ...
            // Use tempUsage to track added slots
            const tempBusy = tempUsage[id][dateKey] || [];
            const totalUsed =
              (schedules[id]?.[dateKey]?.length || 0) + tempBusy.length;

            if (totalUsed >= participantsData[id].threshold) return false;

            return true;
          });

          if (ok) {
            const slotStr = `${format(slotStart)}–${format(slotEnd)}`;
            slots.push(slotStr);

            // Update tempUsage for each participant
            for (const id of selected) {
              if (!tempUsage[id][dateKey]) tempUsage[id][dateKey] = [];
              tempUsage[id][dateKey].push({
